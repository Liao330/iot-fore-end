      ...
      <el-col :span="16">
        <el-card class="card-right" shadow="never">
          <div class="video-block">
            <iframe :src="videoUrls[0]" frameborder="0"></iframe>
            <iframe :src="videoUrls[1]" frameborder="0"></iframe>
          </div>
          <div class="video-block">
            <iframe :src="videoUrls[2]" frameborder="0"></iframe>
            <iframe :src="videoUrls[3]" frameborder="0"></iframe>
          </div>
        </el-card>
      </el-col>
    </el-row>
...
export default {
  ...
  data() {
    return {
      // 接入视频请求的token,vehiclename,vehicleColor,vehicleColor2
      token: "",
      vehiclename: "",
      vehicleColor: "",
      vehicleColor2: "",
      videoUrls: ["", "", "", ""], // 初始化四个视频链接为空
      ...
      // 摄像头视频流
      webRtcServer: null,
      camera_ip: "127.0.0.1:8000", // 这里看需要
      // 背景颜色，文字颜色
      allBgColor: "#ecf2f6",
      allTextColor: "black",
  ...
  methods: {
    ...
    async getVehicleNum() {
      try {
        let res = await getAllCompaniesWithSubsidiaries(this.searchModel); // 假设这里调用后端接口获取数据
        if (res.code === 20000) {
    ...
    async getVehicle() {
      try {
        console.info("条件条件条件条件",this.searchModel)
        let res = await getAllCompaniesWithSubsidiaries(this.searchModel); // 假设这里调用后端接口获取数据
        if (res.code === 20000) {
          const formattedData = res.data.map((company) => ({
            label: company.companyName,
    ...
    handleOnline() {
      ...
      this.searchModel.onlineStatus = 1;
      this.getVehicle(true);
      this.searchModel = {
        userId : this.userId
      };
    ...
    handleOffline() {
      ...
      console.info("离线");
      // 只显示离线车辆
      this.searchModel.onlineStatus = 0;
      this.getVehicle(true);
      this.searchModel = {
        userId : this.userId
      };
    ...
    handleAlarm() {
      ...
      this.searchModel.processStatus = 0;
    ...
    // 树形控件节点请求
    async handleVehicleClick(data) {
      // 在这里处理车辆节点的点击事件，可以访问data中的属性来获取车辆信息
      console.log("点击了车辆节点", data);
      if (data.level === 3) {
        this.searchModel.licenseNumbers = [data.label];
        console.log("点击了车辆节点22222", this.searchModel);
        this.getvehicleDateList();
        this.vehiclename = data.label;
        if (data.vehicleColor === "蓝色") {
          this.vehicleColor = "1";
        } else if (data.vehicleColor === "黄色") {
          this.vehicleColor = "2";
        } else if (data.vehicleColor === "黑色") {
          this.vehicleColor = "3";
        } else if (data.vehicleColor === "白色") {
          this.vehicleColor = "4";
        } else if (data.vehicleColor === "绿色") {
          this.vehicleColor = "5";
        } else if (data.vehicleColor === "黄绿色") {
          this.vehicleColor = "6";
        } else if (data.vehicleColor === "蓝底白字") {
          this.vehicleColor2 = "1";
        } else if (data.vehicleColor === "绿底白字") {
          this.vehicleColor2 = "2";
        } else if (data.vehicleColor === "绿色渐变") {
          this.vehicleColor2 = "3";
        } else if (data.vehicleColor === "绿底黑字") {
          this.vehicleColor2 = "4";
        } else if (data.vehicleColor === "黄绿双拼") {
          this.vehicleColor2 = "5";
        } else if (data.vehicleColor === "黄底黑字") {
          this.vehicleColor2 = "6";
        } else if (data.vehicleColor === "黑底白字") {
          this.vehicleColor2 = "7";
        } else if (data.vehicleColor === "白底黑字") {
          this.vehicleColor2 = "8";
        } else if (data.vehicleColor === "农黄") {
          this.vehicleColor2 = "9";
        } else if (data.vehicleColor === "农绿") {
          this.vehicleColor2 = "10";
        }
        for (let i = 0; i < 4; i++) {
          let apiUrl = `http://220.203.0.15:8083/alonevideo?token=${
            this.token
          }&vehiclename=${this.vehiclename}&ch=${i + 1}&width=424&height=196`;
          ...
          this.videoUrls[i] = apiUrl; // 更新对应的视频链接
        }
        this.vehicleColor = "";
        this.vehicleColor2 = "";
      }
